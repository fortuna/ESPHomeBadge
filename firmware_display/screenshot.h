...
inline void save_screenshot(const char* path) {
  ...
  if (!win) {
    ESP_LOGE("screenshot", "No SDL window found");
    return;
  }
  SDL_Renderer* ren = SDL_GetRenderer(win);
  if (!ren) {
    ESP_LOGE("screenshot", "No SDL renderer found");
    return;
  }
  int w, h;
  if (SDL_GetRendererOutputSize(ren, &w, &h) != 0) {
    ESP_LOGE("screenshot", "SDL_GetRendererOutputSize failed: %s", SDL_GetError());
    return;
  }
  SDL_Surface* surf = SDL_CreateRGBSurface(0, w, h, 32,
      0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);
  if (!surf) {
    ESP_LOGE("screenshot", "SDL_CreateRGBSurface failed: %s", SDL_GetError());
    return;
  }
  if (SDL_RenderReadPixels(ren, nullptr, surf->format->format, surf->pixels,
                           surf->pitch) != 0) {
